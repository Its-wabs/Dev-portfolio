...
const AboutModal = ({ isOpen, onClose }: AboutModalProps) => {
  ...
  useEffect(() => {
  if (isOpen) {
    // Completely remove the background scrollbar
    document.body.style.overflow = "hidden";
  } else {
    // Bring it back when modal is closed
  ...
  const handleExit = () => {
    ...
    gsap.to(contentRef.current, {
      ...
      duration: 0.6,
      ease: "power4.in",
      onComplete: onClose // Change state after animation
    });
  };
      ...
      >
        ...
        <header className="flex flex-col md:flex-row justify-between items-start border-b border-black pb-12 mb-20">
          ...
          <div className="mt-8 md:mt-0 md:text-right font-mono text-[11px] uppercase tracking-widest leading-relaxed">
            ...
            <p className="mt-4 text-[#63938C]">2024_edition</p>
          </div>
        </header>

        {/* BIO */}
        <section className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-32">
          <div className="md:col-span-4 italic font-serif text-2xl border-l-2 border-[#63938C] pl-6">
            "I found out that App systems and blank canvases share the same DNA: they both require a vision for order."
          </div>
          <div className="md:col-span-8 text-xl leading-relaxed text-black/80">
            <p className="mb-6">
            ...
            <p>
              Transitioning into engineering wasn't a pivot, but an expansion. Now, I use TypeScript and React as my brushes to build high-performance systems that don't just work, but feels right. I treat every project like art piece: clean, optimized, and built to last.
            </p>
          </div>
        </section>
        ...
        <section className="mb-32">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[1px] bg-black/10 border border-black/10">
            {principles.map((p, i) => (
              <div key={i} className="bg-[#ebe5d0] p-8 hover:bg-white transition-colors duration-500 group">
                ...
                <p className="text-sm leading-relaxed text-black/60 group-hover:text-black transition-colors">
                  {p.desc}
                </p>
              </div>
            ))}
...
<section className="mb-32">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black/5 border border-black/5">
    {domains.map((domain, i) => (
      <div key={i} className="bg-[#f4f1e6] p-10 flex flex-col justify-between group hover:bg-white transition-colors duration-500">
        <div>
          <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-[#63938C] mb-6 block">
            Domain_0{i+1} // {domain.label}
          </span>
          <h3 className="text-2xl font-black uppercase tracking-tighter mb-8 leading-none">
            {domain.title}
          </h3>
        </div>
        ...
        <div className="space-y-6">
          <div className="flex gap-4 items-start">
            <span className="font-mono text-[9px] text-black/30 mt-1">INFLUENCE</span>
            <p className="text-sm text-black/70 leading-relaxed uppercase tracking-tight font-medium">
              {domain.influence}
            </p>
          </div>
          <div className="flex gap-4 items-start border-t border-black/5 pt-4">
            <span className="font-mono text-[9px] text-[#63938C] mt-1">ENGINEERING_VALUE</span>
